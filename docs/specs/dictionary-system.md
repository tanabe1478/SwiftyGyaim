...
> Last updated: 2026-07-10 (ContextDictのrecency減衰・typo疑い検出CLI — issue #58)
...
### studydictのtypo疑い検出CLI（issue #58）

`Tools/dict/find-suspect-study-entries.py` は studydict のゴミエントリ候補を列挙する（自動削除はしない）。

- `garbage-completion`: 別エントリの語+末尾1文字で、短い方の頻度が支配的（BUG-025 の「してほしいい」型）
- `stale-singleton`: 頻度1かつ `--stale-days`（既定90日）以上未使用
- 削除は変換中の Shift+X またはユーザー辞書エディタから手動で行う
