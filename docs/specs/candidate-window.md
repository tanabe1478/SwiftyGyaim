...
設定キー: `candidateDisplayMode` (Int, 0=list, 1=classic, デフォルト1)。設定画面で変更する値は `~/.gyaim/settings.json` に保存し、既存UserDefaults値がある場合は後方互換のfallbackとして読む。
...
- 設定キー: `aiRerankUseModelForFastContext` (Bool, デフォルトfalse)
