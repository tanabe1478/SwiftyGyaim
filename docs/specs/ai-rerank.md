...
初段は Swift in-process heuristic rerank を使う。さらに、アプリ同梱の `zenz-v3.1-xsmall-gguf` GGUF モデルを bundle resource として持ち、IMEプロセス内で memory-map して保持する。現時点では token inference への接続は未実装で、Python resident server / external command の GPT-2 rerank は後追い補助として残す。
...
`InProcessAIReranker` は `AIRerankBackend` を優先順に試す。現在は `BundledZenzAIRerankBackend` が `BundledZenzRuntime` を通じて `BundledAIRerankModel` を prepare し、このファイルを `Data(contentsOf:options: .mappedIfSafe)` で memory-map してIMEプロセス内で保持する。`ZenzRuntime.rerank(_:)` は llama.cpp token inference の接続点で、現時点では nil を返すため Swift heuristic score に fallback する。モデルが見つからない場合は `HeuristicAIRerankBackend` に fallback する。
...
- 同梱 GGUF モデルは現時点では memory-map までで、`ZenzRuntime.rerank(_:)` の token inference はまだ nil placeholder
