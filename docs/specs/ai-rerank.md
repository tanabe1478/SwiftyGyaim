...
初期実装では Zenzai は使わない。`ku-nlp/gpt2-small-japanese-char` を使う Python resident server / external command を同梱し、SwiftyGyaim 本体は ML runtime やモデルに直接依存しない。
...
- `ku-nlp/gpt2-small-japanese-char` は CC-BY-SA-4.0 ライセンス。アプリ同梱配布する場合はライセンス影響を別途確認する
- 現時点では周辺文脈を渡していない
