...
Tab時の初段 rerank は Swift in-process heuristic で常に実行する。これは HTTP / external command を使わないためプロセス境界がなく、server 未起動でも即時に候補順を補正できる。
...
`BundledAIRerankModel` はこのファイルを `Data(contentsOf:options: .mappedIfSafe)` で memory-map し、IMEプロセス内で保持する。これは llama.cpp / GGUF 推論接続の前段階である。
...
Tab while converting
  -> requestAIRerankIfAvailable()
       ...
       -> Swift in-process heuristic で即 rerank
