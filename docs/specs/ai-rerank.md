...
SwiftyGyaim の既存候補に、Tab 明示起動でローカル複合候補・補完候補・Google Input Tools 候補を加え、ローカル AI reranker で候補順を補正する。

初段は Swift in-process heuristic rerank を使う。さらに、アプリ同梱の `zenz-v3.1-xsmall-gguf` GGUF モデルを bundle resource として持ち、IMEプロセス内で memory-map / llama.cpp context として保持する。`LlamaZenzContext` は tokenization / `llama_decode` / logits 取得を行い、候補文字列の平均 log probability を score に混ぜる。Python resident server / external command の GPT-2 rerank は後追い補助として残す。
...
`InProcessAIReranker` は `AIRerankBackend` を優先順に試す。現在は `BundledZenzAIRerankBackend` が `BundledZenzRuntime` を通じて `BundledAIRerankModel` を prepare し、このファイルを `Data(contentsOf:options: .mappedIfSafe)` で memory-map してIMEプロセス内で保持する。`llama` module が link されている場合は `LlamaZenzContext` が `llama_backend_init` / `llama_model_load_from_file(use_mmap: true)` / `llama_init_from_model` / `llama_model_get_vocab` まで実行し、model/context/vocab を resident にする。`ZenzRuntime.rerank(_:)` は `読み: <hiragana>\n変換:` prompt に続く candidate text の token 平均 log probability を計算し、Swift heuristic score に小さく加算して order を返す。モデルが見つからない場合や context 初期化・decode に失敗した場合は `HeuristicAIRerankBackend` に fallback する。

GPT-2 rerank は opt-in。resident server への直接HTTP接続、または external command を指定した場合だけ、Swift heuristic の後に追加で非同期実行される。
...
Tab while converting
  -> requestAIRerankIfAvailable()
       ...
       -> Google込みでも Swift heuristic で即 rerank
       -> GPT-2 server / external command が設定されていればGoogle込みで後追い再rerank
       -> response order を検証
...
- 同梱 GGUF モデルは llama.cpp decode/logits scoring まで接続済みだが、Zenzai本来の candidate evaluation prompt とはまだ一致していない簡易score
