...
初段は Swift in-process heuristic rerank を使う。さらに、アプリ同梱の `zenz-v3.1-xsmall-gguf` GGUF モデルを bundle resource として持ち、IMEプロセス内で memory-map / llama.cpp context として保持する。現時点では candidate scoring への token inference 接続は未実装で、Python resident server / external command の GPT-2 rerank は後追い補助として残す。
...
- 同梱 GGUF モデルは llama.cpp model/context/vocab 初期化まで接続済みだが、`ZenzRuntime.rerank(_:)` の candidate scoring はまだ nil placeholder
