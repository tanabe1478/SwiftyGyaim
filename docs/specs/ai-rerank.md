...
`InProcessAIReranker` は `AIRerankBackend` を優先順に試す。現在は `BundledZenzAIRerankBackend` が `BundledAIRerankModel` を通じてこのファイルを `Data(contentsOf:options: .mappedIfSafe)` で memory-map し、IMEプロセス内で保持したうえで Swift heuristic score を返す。モデルが見つからない場合は `HeuristicAIRerankBackend` に fallback する。呼び出し元は `InProcessAIReranker` のみを見るため、後続で `BundledZenzAIRerankBackend` の中身を llama.cpp / GGUF 推論へ差し替えられる。
...
- 同梱 GGUF モデルは現時点では memory-map までで、token inference にはまだ使っていない
