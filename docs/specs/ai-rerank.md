...
2. 既存候補集合の表層は**列挙の内側で**スキップする（`excluding:`。スロットを消費しない。後段フィルタ方式は列挙順が通常検索と同一なため全滅する — BUG-028）。モデルの仕事は通常列挙順で沈んだ合成の救済
