...
`InProcessAIReranker` は `BundledAIRerankModel` を通じてこのファイルを `Data(contentsOf:options: .mappedIfSafe)` で memory-map し、IMEプロセス内で保持する。現時点のscore計算は Swift heuristic だが、呼び出し元は `InProcessAIReranker` のみを見るため、後続で llama.cpp / GGUF 推論へ差し替えられる。
...
Tab while converting
  -> requestAIRerankIfAvailable()
       ...
       -> InProcessAIReranker（同梱モデル保持 + Swift heuristic）で即 rerank
