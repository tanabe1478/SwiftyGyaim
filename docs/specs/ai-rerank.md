...
AI rerank は opt-in。resident server への直接HTTP接続、または external command を指定した場合だけ有効になる。
...
- resident server を起動していない場合、HTTP reranker / client script は失敗し、IME 本体は元順に fallback する
- 単発実行版 `gyaim-gpt2-char-rerank.py` は毎回モデルをロードするため、主に protocol 検証用
- IME 本体は timeout で fallback するため入力は止まらないが、rerank が間に合わない場合は元順のまま
- `ku-nlp/gpt2-small-japanese-char` は CC-BY-SA-4.0 ライセンス。アプリ同梱配布する場合はライセンス影響を別途確認する
- 現時点では周辺文脈を渡していない
- Python client process 起動コストを避けるため、実用時は `GYAIM_AI_RERANK_SERVER` / `aiRerankServerURL` による Swift 直接HTTP接続を推奨する
