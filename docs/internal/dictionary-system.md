...
`XX的` と `XX性` も同じ suffix-only モデルで扱う。
...
saigen    再現  普通名詞  普通名詞接続
sei       *性   普通名詞接続  普通名詞接続
```

これにより `chuushouteki -> 抽象的`、`chuushoutekina -> 抽象的な`、`chuushoutekini -> 抽象的に`、`saigensei -> 再現性`、`anzensei -> 安全性`、`gijutsusei -> 技術性` などを固定エントリなしで生成できる。`*的` / `*性` も先頭 `*` により単独入力のトップレベル候補は増やさず、接続経路上でのみ使う。`*性` は `用 + 性 -> 用性` のような短い一般名詞からのノイズを避けるため、一般名詞 `3 -> 4` からは直接接続しない。

### 条件形「ったら」

ログで `owattara -> 終わったら` が候補下位に出ていた。`終わ` は r行系語幹として `outConnection = 16` を持つため、`*ったら` を `inConnection = 16` の内部語尾として追加すると、以下が生成できる。
...
これにより `owattara -> 終わったら`、`kawattara -> 変わったら`、`kaitara -> 書いたら` のような条件形を固定語なしで生成する。`outConnection = 0` として、`終わったらか` のような過剰接続は避ける。
