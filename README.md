...
設定画面で変更した値は `~/.gyaim/settings.json` に保存されます（書き込み先はこのファイルのみ）。旧バージョンが UserDefaults に保存した値は、初回起動時に settings.json へ移行され、それまでは読み取り時のフォールバックとして使われます。全キーと既定値は `docs/specs/settings.md` を参照してください。
